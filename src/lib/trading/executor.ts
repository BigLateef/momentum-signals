...
import { evaluateEligibility, isChainConfiguredForAutoTrade, resolveWalletEligibility } from "./eligibility";
...
export async function executeTradeForSignal(signalId: string): Promise<ExecuteResult> {
  ...
  if (!isChainConfiguredForAutoTrade(signal.chain, config)) {
  ...
  // Dry-run never signs or broadcasts a transaction (guaranteed structurally
  // below — see the `if (config.dryRun)` branch, which returns before either
  // executeJupiterSwap or executePancakeSwap is ever called), so it has no
  // real need for a burner-wallet private key, configuration, or funds.
  // `realWalletConfigured` is the actual, factual check — used for SELL
  // balance lookups and (implicitly, via executeJupiterSwap/
  // executePancakeSwap's own internal checks) live execution.
  // `walletConfigured` is what's reported to the eligibility rules engine —
  // forced true during dry-run so WALLET_NOT_CONFIGURED /
  // INSUFFICIENT_WALLET_BALANCE never block a dry-run evaluation, while live
  // execution (config.dryRun === false) still requires the real check.
  const realWalletConfigured = isWalletConfigured(config.chain);
  const walletConfigured = resolveWalletEligibility(config.dryRun, realWalletConfigured);
  const walletAddress = getWalletPublicAddress(config.chain);

  let tokenBalance: number | null = null;
  if (action === "SELL" && realWalletConfigured) {
    tokenBalance =
      config.chain === "SOLANA"
  ...
  // instead of guessing. `walletConfigured` is forced true during dry-run
  // (see above), so this proceeds without a real wallet — the quote
  // endpoints themselves (Jupiter/PancakeSwap) don't require one for a BUY;
  // Jupiter's quote call never touches a wallet address at all, and
  // PancakeSwap's is a read-only router call. Only the actual swap-signing
  // step later needs a real key, and that's gated separately.
  ...
  const eligibility = evaluateEligibility({
    ...
    dailyLossUsd,
    walletConfigured,
    // Dry-run: forced true alongside walletConfigured above, so
    // INSUFFICIENT_WALLET_BALANCE never blocks a dry-run evaluation either.
    // Live: still an alias of the real walletConfigured check — this app
    // doesn't independently verify the funded *amount* is enough to cover
    // amountConfigured, only that a wallet exists at all; the real
    // insufficient-funds case surfaces as a FAILED execution from the swap
    // attempt itself rather than being pre-empted here.
    walletHasSufficientFunds: walletConfigured,
    tokenBalance,
  });
