...
export async function analyzeToken(opts: AnalyzeOptions): Promise<SafetyReport> {
  ...
  if (isSolana) {
    try {
      rugcheck = await getRugCheckReport(tokenAddress);
      dataSources.rugcheck = rugcheck ? "ok" : "unavailable";
    } catch {
      dataSources.rugcheck = "error";
    }
  } else if (GOPLUS_CHAIN_IDS[chain]) {
    try {
      goplus = await getGoPlusTokenSecurity(chain, tokenAddress);
      dataSources.goplus = goplus ? "ok" : "unavailable";
    } catch {
