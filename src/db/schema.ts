...
export const tokenSafetyReports = pgTable("token_safety_reports", {
  ...
  // Raw provider response text (capped, most-recent-sample-only — not a
  // full audit log), captured specifically to debug provider field-shape
  // mismatches without needing terminal/curl access. See
  // src/lib/safety/providers/rugcheck.ts's top-of-file comment for why this
  // exists: the original RugCheck field mapping was guessed rather than
  // verified against a live response, and this is how that got fixed.
  rawProviderResponses: jsonb("raw_provider_responses"),
