...
import { runPriceRefreshStage } from "@/lib/cycle/prices";
...
// Kept for rollback compatibility. Price-refresh logic now lives in
// src/lib/cycle/prices.ts, shared with /api/cron/market-cycle and
...
async function handle(req: NextRequest) {
  ...
  const { pairsBySignalId, ...result } = await runPriceRefreshStage();
